...
router.put('/activate/:code', function(req, res, next) {
  LOGGER.debug('Activate user with code', req.params.code);
  EEBookRequest(req, 'PUT', '/auth/activate/' + req.params.code).then(function (result) {
    res.send(result);
  }).catch(function (err) {
    return next(err);
  });
